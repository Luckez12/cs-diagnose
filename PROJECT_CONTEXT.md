...
## v15.2 localization (2026-09-24)
- Diagnose user-facing UI, dropdown labels, Live Status history/current status, Important explanations, Plugin Logs attribution and Search messages now resolve via Android string resources using the active CloudStream Activity locale. Default resources are English; Malay (ms) has explicit translations; all other locales fall back to English for Diagnose until translated. Language changes apply when the Diagnose page is reopened after the app changes locale.
- Event keys, `provider` values, URLs, technical Full Trace field names/stage IDs, and unmodified plugin `Log.*` messages are kept as-is for search, copy, correlation and debugging. The Settings Diagnose label now references a translated resource; other official Settings and Logcat UI are unchanged.
- v15.2 patch is a delta against v15.1. No playback/network/provider/extractor instrumentation or behavior changes and no local APK build.
